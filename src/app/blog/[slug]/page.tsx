...
import { ArrowLeft, ArrowRight, MessageCircle, Eye, Tag, Calendar, User, Share2 } from "lucide-react";
import { updates } from "@/lib/content";
import { Button } from "@/components/ui/button";

export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  const resolvedParams = await params;
  const post = updates.find((item) => item.slug === resolvedParams.slug);
    ...
    <article className="w-full relative bg-slate-50 dark:bg-slate-950 min-h-screen pb-24">
      ...
      <section className="relative pt-32 pb-16 lg:pt-40 lg:pb-24 w-full flex items-center justify-center overflow-hidden min-h-[500px]">
        ...
        <div className="absolute inset-0 z-0">
          <Image
            src={post.image || "https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?q=80&w=1600"}
            alt={post.title}
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 text-center text-white space-y-6">
          <Link href="/blog" className="inline-flex items-center text-emerald-400 hover:text-emerald-300 font-bold text-sm uppercase tracking-wider mb-4 transition-colors">
            <ArrowLeft className="w-4 h-4 mr-2" /> Back to Activities
          </Link>

          <div className="flex justify-center gap-2 mb-4">
            <span className="bg-emerald-600 px-3 py-1 rounded-full text-xs font-bold tracking-wider uppercase">
              {post.category}
            </span>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 text-sm md:text-base text-slate-300 font-medium">
            ...
            <span className="w-1.5 h-1.5 rounded-full bg-slate-600"></span>
            <span className="flex items-center"><Eye className="w-4 h-4 mr-2" /> {post.views} Views</span>
          </div>
        </div>
      </section>
      ...
      <section className="relative z-20 max-w-4xl mx-auto px-4 sm:px-6 -mt-10 md:-mt-16">
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-8 md:p-12 border border-slate-200 dark:border-slate-800">
          ...
          {/* Article Body */}
          <div className="prose prose-base sm:prose-lg dark:prose-invert prose-emerald max-w-none mb-12">
            <p className="text-lg sm:text-xl leading-relaxed text-slate-600 dark:text-slate-300 font-light mb-8">
              {post.excerpt}
            </p>
            <p className="text-slate-800 dark:text-slate-200 leading-relaxed text-base sm:text-lg">
              {post.content || "Detailed content for this report will be loaded here. Currently viewing the placeholder text for the expanded activity overview which explains the impact and methodology used in the field."}
            </p>
          </div>
          ...
          <div className="border-t border-slate-100 dark:border-slate-800 pt-10 mt-10">
            ...
            <div className="bg-slate-50 dark:bg-slate-950 p-4 sm:p-6 rounded-2xl border border-slate-100 dark:border-slate-800 mb-8">
              ...
              <textarea
                className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 min-h-[120px] focus:ring-2 focus:ring-emerald-500 focus:outline-none resize-none mb-4 text-sm sm:text-base"
                placeholder="Share your thoughts about this impact report..."
              ></textarea>
              <Button className="w-full sm:w-auto bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-6 rounded-xl">
                Post Comment
              </Button>
            </div>
      ...
      {relatedPosts.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 mt-16 sm:mt-20">
          <h3 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white mb-6 sm:mb-8">Related Activities in <span className="text-emerald-600">{post.category}</span></h3>
          <div className="grid md:grid-cols-2 gap-6 sm:gap-8">
            {relatedPosts.map((related, idx) => (
              <Link href={`/blog/${related.slug}`} key={related.slug} className="group bg-white dark:bg-slate-900 rounded-3xl overflow-hidden border border-slate-200 dark:border-slate-800 dark:hover:-translate-y-1 transition-all duration-300 flex flex-col sm:flex-row">
                <div className="w-full sm:w-1/3 h-48 sm:h-auto bg-slate-100 dark:bg-slate-800 overflow-hidden shrink-0">
                  <Image
                    src={related.image || `https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?q=80&w=400&sig=${idx + 10}`}
                    alt={related.title}
                ...
                <div className="w-full sm:w-2/3 p-5 sm:p-6 flex flex-col justify-center">
                  <span className="text-xs font-bold text-emerald-600 mb-2 block">{related.date}</span>
                  <h4 className="text-lg font-bold text-slate-900 dark:text-white group-hover:text-emerald-600 transition-colors line-clamp-2 mb-2">{related.title}</h4>
                  <span className="text-sm font-bold text-slate-500 group-hover:text-emerald-600 flex items-center mt-auto">Read <ArrowRight className="ml-1 w-3 h-3 group-hover:translate-x-1 transition-transform" /></span>
                </div>
              </Link>
            ))}
          </div>
