        ...
        <div className="flex flex-col lg:flex-row gap-10">
          ...
          <div className="flex-1 space-y-8">
            {paginatedUpdates.length === 0 ? (
              <div className="bg-white dark:bg-slate-900 rounded-3xl p-12 text-center border border-slate-200 dark:border-slate-800 ">
                ...
                <button onClick={() => { setSearchQuery(''); setSelectedCategory('All'); setSelectedTag(null); }} className="mt-6 text-emerald-600 font-bold hover:underline">Clear all filters</button>
              </div>
            ) : (
              paginatedUpdates.map((update, idx) => (
                <Link href={`/blog/${update.slug}`} key={update.slug} className="group bg-white dark:bg-slate-900 rounded-3xl overflow-hidden border border-slate-200 dark:border-slate-800 dark:hover:-translate-y-1 transition-all duration-300 flex flex-col md:flex-row">
                  <div className="relative md:w-2/5 h-[240px] sm:h-[300px] md:h-auto overflow-hidden bg-slate-100 dark:bg-slate-800 flex-shrink-0">
                    ...
                    />
                    <div className="absolute top-4 left-4 z-10 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md px-3 py-1 rounded-full text-xs font-bold text-emerald-600 border border-emerald-100 dark:border-emerald-900/50">
                      {update.category}
                    </div>
                  </div>
                  <div className="p-8 md:w-3/5 flex flex-col justify-center">
                    ...
                    <h4 className="text-2xl md:text-3xl font-bold leading-tight group-hover:text-emerald-600 transition-colors mb-4 text-slate-900 dark:text-white">{update.title}</h4>
                    ...
                    <div className="text-sm font-bold text-emerald-600 flex items-center w-fit group-hover:translate-x-1 transition-transform mt-auto">
          ...
          <div className="lg:w-80 space-y-8 flex-shrink-0">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 ">
              <h4 className="font-bold text-lg mb-4 text-slate-900 dark:text-white flex items-center"><Search className="w-4 h-4 mr-2 text-emerald-600" /> Search</h4>
              <Input
                placeholder="Search activities..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="bg-slate-50 dark:bg-slate-950 border-slate-200 dark:border-slate-800 focus-visible:ring-emerald-500 rounded-xl h-11"
              />
            </div>

            {/* Categories */}
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 ">
              <h4 className="font-bold text-lg mb-4 text-slate-900 dark:text-white flex items-center"><Filter className="w-4 h-4 mr-2 text-emerald-600" /> Categories</h4>
              <div className="flex flex-col gap-2">
                {categories.map(cat => (
                  <button
                    key={cat}
                    onClick={() => setSelectedCategory(cat)}
                    className={`flex items-center justify-between px-4 py-2.5 rounded-xl text-sm font-medium transition-colors ${selectedCategory === cat ? 'bg-emerald-50 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}
                  >
                    {cat}
                    <span className={`text-xs px-2 py-0.5 rounded-full ${selectedCategory === cat ? 'bg-emerald-100 dark:bg-emerald-900 text-emerald-700 dark:text-emerald-400' : 'bg-slate-100 dark:bg-slate-800 text-slate-500'}`}>
            ...
            {/* Top 5 Activities */}
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 ">
              <h4 className="font-bold text-lg mb-4 text-slate-900 dark:text-white flex items-center"><Eye className="w-4 h-4 mr-2 text-emerald-600" /> Popular Posts</h4>
              <div className="space-y-4">
                {topActivities.map((post, idx) => (
                  <Link href={`/blog/${post.slug}`} key={post.slug} className="group flex gap-4 items-center">
                    <span className="text-3xl font-bold text-slate-200 dark:text-slate-800 group-hover:text-emerald-200 dark:group-hover:text-emerald-900 leading-none">0{idx + 1}</span>
                    <div>
                      <h5 className="text-sm font-bold text-slate-900 dark:text-white group-hover:text-emerald-600 transition-colors line-clamp-2 leading-snug">{post.title}</h5>
            ...
            {/* Popular Tags */}
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 ">
              <h4 className="font-bold text-lg mb-4 text-slate-900 dark:text-white flex items-center"><Tag className="w-4 h-4 mr-2 text-emerald-600" /> Tags</h4>
              <div className="flex flex-wrap gap-2">
                {allTags.map(tag => (
                  <button
                    key={tag}
                    onClick={() => setSelectedTag(selectedTag === tag ? null : tag)}
                    className={`text-xs uppercase tracking-wider font-bold px-3 py-1.5 rounded-lg transition-colors ${selectedTag === tag ? 'bg-emerald-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700'}`}
                  >
                    {tag}
                  </button>
                ))}
